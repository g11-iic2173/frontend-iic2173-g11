...
export default function PropertyDetailPage() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (res?.data?.deposit_url && res?.data?.deposit_token) {
        navigate("/confirm-purchase", {
          state: {
            ...
            property_url: property.url,
            amount: property.ten_percent_clp,
            title: property.name,
      ...
      alert(e?.response?.data?.error || "No se pudo procesar la solicitud de compra");
    }
  };
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <p>
        Precio del agendamiento (10% convertido a CLP):{" "}
        <strong>
          {property.ten_percent_clp?.toLocaleString("es-CL")} CLP
        </strong>
      </p>

      <button disabled={!canBuy} onClick={handleBuy}>
        {canBuy ? "Comprar agendamiento" : "No hay cupos disponibles"}
      </button>
