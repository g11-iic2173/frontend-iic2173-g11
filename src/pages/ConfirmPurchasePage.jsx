...
export default function ConfirmPurchasePage() {
  ...
  // 🔥 Eliminado: const type = state?.type || "property";
  ...
  const handleSubmit = async (e) => {
    ...
    if (!property_url || !property_id || !deposit_token) {
      alert("Faltan datos requeridos para crear el intent de pago.");
      return;
    }
    ...
    try {
      ...
      await axios.post(
        ...
        { property_url, property_id, deposit_token },
        { headers }
      );
  ...
  if (!depositUrl || !depositToken) {
    return (
      <div className="p-6">
        ...
        <p className="mt-4">No se encontró información de pago.</p>
        <button onClick={() => navigate(-1)} className="px-3 py-2 bg-gray-200 rounded mt-4">
          Volver
        </button>
      </div>
    );
